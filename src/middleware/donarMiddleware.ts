...
export const addDonarMiddleware: RequestHandler = (req: any, res: Response, next: any) => {
  const userType = req.userType;
  if (userType === 'admin') {
    const reqData = req.body;
    const name =
      typeof reqData.name === 'string' && reqData.name.trim().length > 2 ? reqData.name : false;
    const blood =
      typeof reqData.blood === 'string' && bloodGroup.indexOf(reqData.blood) > -1
        ? reqData.blood
        : false;
    const mobile =
      typeof reqData.mobile === 'string' && reqData.mobile.trim().length === 11
        ? reqData.mobile
        : false;
    const email =
      typeof reqData.email === 'string' && reqData.email.trim().length > 2 ? reqData.email : false;
    const dob =
      typeof reqData.dob === 'string' && checkIsEligible(reqData.dob, 6575) ? reqData.dob : false;
    const aboutMe =
      typeof reqData.aboutMe === 'string' && reqData.aboutMe.trim().length > 2
        ? reqData.aboutMe
        : false;
    const address =
      typeof reqData.address === 'string' && reqData.address.trim().length > 2
        ? reqData.address
        : false;
    const status =
      typeof reqData.status === 'string' && statusList.indexOf(reqData.status) > -1
        ? reqData.status
        : false;
    ...
    next('Your are not authorized to add new donar');
  }
};
...
export const donarDetailsMiddleware: RequestHandler = (req, res, next) => {
  const donarId =
    ...
    ObjectId.isValid(req.params.donar)
      ? req.params.donar
      : false;
...
export const updateDonarStatusMiddleware: RequestHandler = (req, res, next) => {
  const donarId =
    ...
    req.body.donar.length === 24 &&
    ObjectId.isValid(req.body.donar)
      ? req.body.donar
      : false;
  const status =
    typeof req.body.status === 'string' && statusList.indexOf(req.body.status) > -1
      ? req.body.status
      : false;
...
// update donar middleware
export const updateDonarMiddleware: RequestHandler = (req, res, next) => {
  if (req.body.hasOwnProperty('mobile') || req.body.hasOwnProperty('email')) {
    next('user phone and email are not allow to update');
  } else {
    const reqData = req.body;
    ...
    if (donarId && (name || blood || dob || aboutMe || address || status)) next();
    else next('user id invalid');
  }
};
